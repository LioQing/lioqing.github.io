...
use crate::{
    frame::FrameMetadata, line_segment::LineSegments, meta_field::MetaField, meta_image::MetaImage,
    meta_shape::MetaShapes,
};
...
#[derive(Debug)]
pub struct MetaFieldImageProcessor {
    workgroup_size: UVec2,
    compute_pipeline: wgpu::ComputePipeline,
    bind_group_layout: wgpu::BindGroupLayout,
}

impl MetaFieldImageProcessor {
    pub fn new(device: &wgpu::Device, meta_field: &MetaField) -> Self {
        let workgroup_size = {
            ...
            log::debug!("Chosen workgroup size for MetaFieldImageProcessor: {max_size:?}");

            max_size.expect("device must support workgroup size of at least 4x4")
        };

        let shader_module = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("Meta Field Image Processor Shader Module"),
            source: wgpu::ShaderSource::Wgsl(
                include_str!("shader/meta_field_image_process.wgsl").into(),
            ),
        });

        let bind_group_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("Meta Field Image Processor Bind Group Layout"),
            entries: &[
                wgpu::BindGroupLayoutEntry {
                    binding: 0,
                    visibility: wgpu::ShaderStages::COMPUTE,
        ...
        let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("Meta Field Image Processor Pipeline Layout"),
            bind_group_layouts: &[&bind_group_layout],
            push_constant_ranges: &[],
        });

        let compute_pipeline = device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
            label: Some("Meta Field Image Processor Compute Pipeline"),
            layout: Some(&pipeline_layout),
            module: &shader_module,
            entry_point: Some("main"),
            compilation_options: wgpu::PipelineCompilationOptions {
    ...
    pub fn process(
        ...
        let bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("Meta Field Image Processor Bind Group"),
            layout: &self.bind_group_layout,
            entries: &[
                wgpu::BindGroupEntry {
                    binding: 0,
                ...
                wgpu::BindGroupEntry {
                    binding: 2,
                    resource: meta_image.buffer().as_entire_binding(),
                },
                wgpu::BindGroupEntry {
                    binding: 3,
                    resource: wgpu::BindingResource::TextureView(
        ...
        let mut compute_pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
            label: Some("Meta Field Image Processor Compute Pass"),
            ..Default::default()
        });
        ...
        let resolution = meta_field.resolution();
        compute_pass.dispatch_workgroups(
            resolution.x.div_ceil(self.workgroup_size.x),
            resolution.y.div_ceil(self.workgroup_size.y),
            1,
